...
namespace Simple.Testing.ReSharperRunner
{
	...
	public static class Guard
	{
		...
		public static bool BeginInvokeAndWait<T>(this IThreading threading, string description, global::System.Func<T> invoker, out T value, params WaitHandle[] waitHandles)
		{
			var guard = threading.ReentrancyGuard;
			var disp = guard.Dispatcher;
			T returnValue = default(T);
			ManualResetEvent finished = new ManualResetEvent(false);
			disp.BeginOrInvoke(description, () => guard.Execute(description, () =>
			{
				returnValue = invoker();
				finished.Set();
			}));
